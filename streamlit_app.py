...
if not st.session_state.authenticated:
    ...
    st.stop()
# ตัวอย่างการวางในโค้ดหลักของบี๋
if st.session_state.get("login_successful"):  # ถ้าล็อคอินผ่านแล้ว
    
    # --- 1. ใส่เพลงไว้ที่แถบข้างๆ (Sidebar) ---
    with st.sidebar:
        st.write("### 🎧 Our Playlist")
        # เช็คอีกทีว่าชื่อไฟล์ในเครื่องกับในโค้ดตรงกันนะจ๊ะ
        st.audio("bg_music.mp3") 
        st.info("เปิดเพลงคลอไปด้วยนะบี๋ ❤️")
        
    # --- 2. ส่วนเนื้อหาแอปของบี๋ (พวกนาฬิกา, Quiz, ของขวัญ) ---
    render_main_app() # หรือโค้ดหน้าต่างๆ ของบี๋
# --- จัดการเมนู ---
